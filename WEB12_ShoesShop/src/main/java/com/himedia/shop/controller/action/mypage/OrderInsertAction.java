...
public class OrderInsertAction implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(mvo == null) {
			...
			// 1. 장바구니의 물건을 일괄주문할 때-> 아이디로 장바구니를 검색
			...
			// 2. 방금 orders 테이블에 추가된 레코드의 oseq를 조회합니다.
			int oseq = odao.lookupMaxOseq(mvo.getUserid());
			
			// 3. list2 에 있는 물건들과 oseq 를 이용해서 order_detail 테이블에 레코드를 추가합니다.
			...
			// 5. 방금 주문에 성공한 주문 번호를 갖고 주문 내역을 다시 조회해서 jsp로 이동합ㅂ니다.
