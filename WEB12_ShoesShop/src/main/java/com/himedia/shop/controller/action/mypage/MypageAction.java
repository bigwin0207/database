...
public class MypageAction implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 진행중인 주문내역
		// 스니커즈 외 1건  가격: 25000 ...	상세보기
		// 부츠 포함 2건 	  가격: 30000 ...	상세보기
		// 슬리퍼 포함 2건  가격: 30000 ...	상세보기
		...
		if(mvo == null) {
			...
			// 현재 로그인 한 사용자의 배송 전 주문 내역을 표시합니다.
			// 한번에 2~3개의 상품을 3회에 걸쳐 주문했다면
			// 진행중인 주문 내역은 3행이 표시됩니다.( 주문 번호별(oseq) 표시)
			
			// 그리고 그들에게는 상세보기 메뉴가 있어서 그 주문에 어떤 상품이 주문 된건지 상세하게 펼쳐볼 수 있다.
			
			// 1. mypage.jsp 에 최종 전달되서 화면에 보여질 리스트를 생성합니다.
			ArrayList<OrderVO> finalList = new ArrayList<OrderVO>();
			// finalList.get(0).getName() -> "XXXX 외 2건" 와 같이 구성됩니다.
			
			// order_view 에서 로그인 유저의 아이디로 아직 발송 전 result 가 1인 것만 검색하면
			// 4 4 4 5 5 6 <- 조회된 주문번호(oseq)들
			// 나에게 지금 필요한건 4 5 6
			// 위와 같은 결과를 얻으려면 sql 문에 distinct 키워드를 써서 조회해 옵니다.
			...
			for(Integer oseq : oseqList) {
				// 과정 : 주문번호로 주문 목록을 조회(order_view)에서 조회
				// 4번 주문의 2개의 상품에서 첫번째 상품의 상품이름을 "상품이름 포함 1 건"으로 변경
