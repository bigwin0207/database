...
<% 
	...
	// 서버에서 JSP 파일로 전달되는 배열이나 ArrayList 는 보통 request의 Attribute 에 담겨옵니다.
...
<!-- 
 	items="" : 반복실행에 이용할 리스트 또는 배열을 지정(request 나 session 에 저장된 키값)
 ...
 <table border="1" style="width:700px; text-align:center">
	...
	<c:forEach items="${mList}" var="movie" varStatus="state">
		<tr><td>${state.index}</td><td>${state.index}</td><td>${movie}</td></tr>
	</c:forEach>
	</table>
<!-- 
 			...
 			<c:choose>
 				<c:when test="${state.first}">
 						<tr style="font-weight:bold; color: red;">
 							<td>${state.index}</td><td>${state.index}</td><td>${movie}</td>
 						</tr>
 				</c:when>
...
<!-- 
${state.first} : 현재 반복이 첫번째 이면 true 리턴
...
<br /><br />
 <table border="1" style="width:50%; text-align:center;" align="left">
	...
	<c:forEach  var="cnt" begin="7" end="10" varStatus="state">
		<tr><td>${state.index}</td><td>${state.count}</td><td>${cnt}</td></tr>
	</c:forEach>
</table>
