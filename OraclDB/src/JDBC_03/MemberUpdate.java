...
public class MemberUpdate {
	...
	public static void main(String[] args) throws ParseException {
		...
		System.out.print("수정할 멤버의 멤버번호를 입력하세요 : ");
		...
		System.out.print("수정할 전화번호를 입력하세요. (수정하지 않으려면 엔터만 입력) -> ");
		input = sc.nextLine();
		if( !input.equals("") ) mdto.setPhone( input );
		
		System.out.printf("성별 : %s\n" , mdto.getGender() );
		System.out.print("수정할 성별을 입력하세요. (수정하지 않으려면 엔터만 입력) -> ");
		input = sc.nextLine();
		if( !input.equals("") ) mdto.setGender( input );

		System.out.printf("보너스포인트 : %s\n" , mdto.getBpoint() );
		System.out.print("수정할 포인트 점수를 입력하세요. (수정하지 않으려면 엔터만 입력) -> ");
		input = sc.nextLine();
