...
public class MainAction implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 1. 페이지 구성요소들을 계산하기 위해 총레코드 갯수부터 조회
		int count = bdao.getAllCount();
		System.out.println("레코드 갯수 : " + count);
		
		// 2. 조회한 레코드갯수를 Paging 객체의 totalCount 에 입력 (setTotalCount()이용)
		paging.setTotalCount(count); // 이때 calPaging()이 호출되서 모든 멤버변수가 계산됩니다.
		
		// 3. Paging 객체의 startNum(시작 레코드 번호) 부터 displayRow(한화면 당 레코드 갯수)만큼
		// 조회합니다.
