...
select  a.rantdate, a.bnum, b.subject , a.mnum, a.discount from rentlist a, booklist b
where a.bnum = b.booknum;

--rentlist의 대여일자, 대여도서번호, 대여회원번호, 할인금액을 출력하되, 회원의 이름을 대여회원번호 옆에 출력하세요
select a.rantdate , a.bnum, a.mnum,b.name, a.discount from rentlist a, memberlist b
where a.mnum = b.membernum;

--rentlist의 대여일자, 대여도서번호, 대여회원번호, 할인금액을 출력하되, 회원의 이름을 도서번호와 회원번호 옆에 출력하세요
select a.rantdate , a.bnum, b.subject, a.mnum, c.name, a.discount, b.rentprice-a.discount as "매출액" from rentlist a, booklist b, memberlist c
...
select a.booknum, a.subject, b.rantdate from booklist a, rentlist b
where a.booknum = b. bnum(+)

--emp 테이블에는 deptno가 40인 레코드가 하나도 없습니다. 그래서 조인된 결과 에는 40번 부서의 이름도 loc도 표시가 안됩니다.
