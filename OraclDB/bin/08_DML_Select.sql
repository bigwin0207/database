...
-- 출파년도가 2020년 이후인 도서의 제목과 출판년도를 조회
...
select subject as "제목", rentprice * 10 - inprice as "10회대여 마진" from booklist; --FROM keyword not found where expected --from 앞에 띄어쓰기 때문에 에러
--큰 따옴표로 묶어준다.
...
-- 검색조건 : update 와 delete 에서 사용하던 where 와 사용방식이 똑같습니다.

-- memberlist 테이블에서 이름이 '홍'으로 시작하는 회원의 모든 회원정보 출력
select*from memberlist where name like '홍%';
-- memberlist 테이블에서 1983 년도 이후로 태어난 회원의 모든 회원정보
...
-- booklist 에서 도서 제목에 두번째 글자가 '것'인 도서정보
...
-- 방법 #2
select*from emp where deptno <> 40; -- 40과 다른

-- 방법 #3
select*from where deptno in(10,20,30);
...
-- asc : 오른차순 정렬, 쓰지 않으면 기본 오름차순 정렬로 실행됩니다.
-- desc : 내림차순 정렬, 내림차순 정렬을 위해서는 반드시 필드명 뒤에 써야하는 키워드 입니다.

-- emp 테이블에서 
-- sal이 1000 이상인 데이터를 ename 의 오른차순으로 정렬하여 조회
select*from emp where sal >= 1000 order by ename; -- 오름차순 일 때 asc는 생략가능
...
select*from emp order by jop desc, hiredare desc;
-- 두가지 이상의 정렬 기준이 필요하다면 위와 같이 컴마(,)로 구분해서 두가지 기준을 지정해주며,
-- 위의 예제로 봤을때 job 으로 1차 내림차순 정렬하고, 같은 job 값들 사이에 hiredate 로 내림차순 정렬합니다.