...
-- 자료형 : booknum(문자5자리), subject(문자30), makeyear(숫자 4),
...
--                                 서로 다른값(유일한값)들을 갖고있어 레코드들을 유일하게 구분해 낼 수 있는 필드
...
--               memberNum : PrimaryKey - 테이블 레벨로 설정
...
-- 예를 들면 booklist 에 존재 하지 않은 도서의 번호가 rentlist 의 빌려간 도서의 번호로 등록되지 못하게 하는 것을 말합니다.
...
create table rentlist(
	idx NUMBER(3), -- 대여기록번호 
	rent_date date default sysdate,  --대여날짜 default sysdate 오늘날짜 현재시간이 자동으로 나옴
...
-- constraint fk2 foreign key(mnum) references memberlist(membernum) 
-- memberlist 에 membernum이 기본키어야 한다.
