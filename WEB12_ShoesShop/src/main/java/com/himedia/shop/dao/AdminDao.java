...
public class AdminDao {
	...
	public int getAllCount(String tablename, String fieldname, String key) {
		int count = 0;
		con = Db.getConnection();
		String spl ="select count(*) as cnt from " + tablename
	...
	public ArrayList<OrderVO> adminOrderList(Paging paging, String key) {
		 ...
		 con = Db.getConnection();
		 String sql ="select * from order_view" +" where pname like concat('%', ?, '%')"
				 		+ " order by result, indate desc, odseq desc limit ? offset ?"; //aesc 는 생략하면 적용
	...
	public void updateOrderResult(int odseq) {
		String sql = "update orders_detail set result='2' where odseq= ?";
		con=Db.getConnection();
		try {
			pstmt = con.prepareStatement(sql);
