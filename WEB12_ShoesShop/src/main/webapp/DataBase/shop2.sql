...
values('one','1111','김나리','133-110','서울시 성동구 성수동 1가','1번지21호','017-777-7777','acc@abc.com');
insert into member(userid, pwd, name, zip_num, address1, address2, phone, email)
values('two','2222','김길동','130-120','서울시 송파구 잠실2동','리센츠아파트 201-505','011-032-7777','aac@abc.com');
...
-- cart 안의 pseq로 상품이름과 그리고 userid로 사용자 이름을 함께 조회하는 view를 생성합니다
...
from orders o, orders_detail d, product p, member m
...
select * from orders where oseq=1;

select * from orders_detail where oseq=3;

update orders_detail set result ='2' where odseq in(5,6,11,12);
